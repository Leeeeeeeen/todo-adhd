...
export default function LifeCountdown() {
  ...
  const currentProgress = mode === 'goal' ? progress : weekProgress

  return (
    <div className="bg-gray-800/60 backdrop-blur border border-gray-700 rounded-2xl p-6">
      <div className="flex items-center justify-between mb-3">
        <h2 className="text-sm font-medium text-gray-400 uppercase tracking-wider">
          {mode === 'goal' ? '目標カウントダウン' : '今週の残り時間'}
        </h2>
        <div className="flex items-center gap-2">
          <span className="text-xs text-gray-500">{currentProgress}% 経過</span>
      ...
      {mode === 'goal' ? (
        ...
        <>
          <div className="mb-1">
            ...
            <span className="text-gray-500 text-sm ml-2">日</span>
            <span className="text-xl font-bold text-purple-400 font-mono tabular-nums ml-3">
              {String(weekRemainH).padStart(2, '0')}:{String(weekRemainM).padStart(2, '0')}
            </span>
          </div>

          <div className="text-gray-500 text-xs mb-4">日曜23:59まで</div>
